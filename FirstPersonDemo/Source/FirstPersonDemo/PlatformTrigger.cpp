...
APlatformTrigger::APlatformTrigger()
{
	...
	if (!ensure(TriggerVolume != nullptr)) return; // return from the method if TriggerVolume is a nullptr // Prevents the editor from crashing if the pointer is null
...
{
	for (AMovingPlatform* p : platforms)
	{
		p->AddActiveTrigger();
	}
}
